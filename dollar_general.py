...
# Dollar General-specific token
DOLLAR_GENERAL_ACCESS_TOKEN = "00be606cd7cb8b0cf999e3c7b038a2fe"
...
def scrape_dollar_general(store_code="19052"):
    """Scrape Dollar General flyer and product data."""
    pub_url = "https://dam.flippenterprise.net/flyerkit/publications/dollargeneral"
    pub_params = {
        "locale": "en",
        "access_token": DOLLAR_GENERAL_ACCESS_TOKEN,
    ...
    for idx, f in enumerate(flyers, 1):
        ...
        # Create base dollar_general folder
        base_folder = Path("dollar_general")
        base_folder.mkdir(exist_ok=True)
        
        # Create specific flyer folder inside dollar_general/
        folder_name = f"DollarGeneral_{safe_flyer_name}_{from_fmt}-{to_fmt}"
        folder_path = base_folder / folder_name
        folder_path.mkdir(exist_ok=True)

        flyer_base_name = f"DollarGeneral_{safe_flyer_name}_{from_fmt}-{to_fmt}"
...
if __name__ == "__main__":
    store_code = "19052"
    print(f"🚀 Starting Dollar General scraper for store: {store_code}\n")
    start_time = time.time()
    data = scrape_dollar_general(store_code)
