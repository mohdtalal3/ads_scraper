...
import json
# CVS-specific token
CVS_ACCESS_TOKEN = "d005d9bbce4a77a9f3908b83e38aae19"
...
# ---------- CVS Scraper ----------
def scrape_cvs(store_code="808"):
    """Scrape CVS flyer and product data."""
    pub_url = "https://dam.flippenterprise.net/flyerkit/publications/cvspharmacy"
    pub_params = {
        "locale": "en",
        "access_token": CVS_ACCESS_TOKEN,
    ...
    for idx, f in enumerate(flyers, 1):
        ...
        # Create base CVS folder
        base_folder = Path("CVS")
        base_folder.mkdir(exist_ok=True)
        
        # Create specific flyer folder inside CVS/
        folder_name = f"CVS_{safe_flyer_name}_{from_fmt}_{to_fmt}"
        folder_path = base_folder / folder_name
        folder_path.mkdir(exist_ok=True)

        flyer_base_name = f"CVS_{safe_flyer_name}_{from_fmt}_{to_fmt}"
...
if __name__ == "__main__":
    store_code = "808"
    print(f"🚀 Starting CVS scraper for store: {store_code}\n")
    start_time = time.time()
    data = scrape_cvs(store_code)
