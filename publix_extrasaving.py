...
def get_promotions(campaign_id, store_code):
    ...
    for p in promos:
        ...
        if "extra saving" in flyer_name.lower():
            enriched.append({
    ...
    print(f"\n🎯 Filtered to {len(enriched)} Extra Savings promotion(s).")
    return enriched
...
def get_publix_weekly_ad(store_code, campaign_id, promotions):
    ...
    for (flyer_name, promotion_type_id, valid_from, valid_to), deals in grouped.items():
        if "extra saving" not in flyer_name.lower():
            print(f"  ⏭️  Skipping non-Extra Savings flyer: {flyer_name}")
            continue
