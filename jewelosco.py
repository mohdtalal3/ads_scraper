...
import json
# Jewel-Osco-specific token
JEWELOSCO_ACCESS_TOKEN = "aedcc3dc3afc882f187dbdd32b6db995"
...
# ---------- Jewel-Osco Scraper ----------
def scrape_jewelosco(store_code="3441"):
    """Scrape Jewel-Osco flyer and product data."""
    pub_url = "https://dam.flippenterprise.net/flyerkit/publications/jewelosco"
    pub_params = {
        "locale": "en",
        "access_token": JEWELOSCO_ACCESS_TOKEN,
    ...
    for idx, f in enumerate(flyers, 1):
        ...
        # Create base Jewel-Osco folder
        base_folder = Path("JewelOsco")
        base_folder.mkdir(exist_ok=True)
        
        # Create specific flyer folder inside JewelOsco/
        folder_name = f"JewelOsco_{safe_flyer_name}_{from_fmt}-{to_fmt}"
        folder_path = base_folder / folder_name
        folder_path.mkdir(exist_ok=True)

        flyer_base_name = f"JewelOsco_{safe_flyer_name}_{from_fmt}-{to_fmt}"
...
if __name__ == "__main__":
    store_code = "3441"
    print(f"🚀 Starting Jewel-Osco scraper for store: {store_code}\n")
    start_time = time.time()
    data = scrape_jewelosco(store_code)
